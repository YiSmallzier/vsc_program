...
stmt    : matched_stmt {  printf("stmt->matched_stmt\n"); }
        | unmatched_stmt {printf("stmt->unmatched_stmt\n");   }
        ;

matched_stmt:   ID '=' expr ';'                  { printf("产生式：stmt->id = expr;\n"); }
        | IF '(' bool ')' matched_stmt ELSE matched_stmt   { printf("产生式：stmt->if (bool) stmt esle stmt\n"); }
        | WHILE '(' bool ')' matched_stmt          { printf("产生式：stmt->while (bool) stmt\n"); }
        | DO matched_stmt WHILE '(' bool ')' ';'   { printf("产生式：stmt->do stmt while (bool)\n"); }
        | BREAK  ';'                       { printf("产生式：stmt->break ;\n"); }
        | block                            { printf("产生式：stmt->block\n"); }
        ;

unmatched_stmt: IF '(' bool ')' stmt {printf("unmatched->if (bool) stmt\n");}
        | IF '(' bool ')' matched_stmt ELSE unmatched_stmt {printf("unmatched->if (bool) stmt else stmt\n");    }
        ;
